...
final class AppModel: ObservableObject {
    ...
    init(
        ...
        statusMessage = L10n.t("Ready", zh: "准备就绪")
    ...
    func discardSelectedProfileChanges() {
        ...
        statusMessage = L10n.t("Discarded unsaved changes", zh: "已放弃未保存的更改")
    }
    ...
    func loadSelectedCredential() {
        ...
        do {
            ...
            statusMessage = L10n.t("Could not read the API key: {0}", zh: "无法读取 API Key：{0}", error.localizedDescription)
        }
    }
    ...
    func addProfile() {
        ...
        statusMessage = L10n.t("Added a new endpoint", zh: "已添加新端点")
    }
    ...
    func deleteSelectedProfile() {
        ...
        guard profiles.count > 1,
            ...
            statusMessage = L10n.t("Keep at least one endpoint", zh: "至少需要保留一个端点")
            return
        }
        ...
        statusMessage = L10n.t("Deleted endpoint “{0}”", zh: "端点“{0}”已删除", removed.displayName)
    }
    ...
    func saveSelectedProfile() -> Bool {
        do {
            ...
            guard !name.isEmpty else {
                statusMessage = L10n.t("Enter an endpoint name", zh: "请填写端点名称")
                return false
            }
            ...
            statusMessage = L10n.t("Endpoint “{0}” was saved securely", zh: "端点“{0}”已安全保存", name)
            return true
        } catch {
    ...
    func testEndpoint() {
        ...
        isBusy = true
        statusMessage = L10n.t("Testing the model API…", zh: "正在测试模型接口…")
            ...
            do {
                ...
                repeat {
                    ...
                    guard (200..<300).contains(http.statusCode) else {
                        statusMessage = L10n.t("Endpoint returned HTTP {0}", zh: "端点返回 HTTP {0}", "\(http.statusCode)")
                        return
                    }
                ...
                if Task.isCancelled || (error as? URLError)?.code == .cancelled {
                    statusMessage = L10n.t("Model sync was cancelled", zh: "模型同步已取消")
                } else {
                    statusMessage = L10n.t("Connection failed: {0}", zh: "连接失败：{0}", error.localizedDescription)
                }
            }
        }
    }
    ...
    func testAllModels() {
        ...
        isBusy = true
        statusMessage = L10n.t("Testing {0} models one by one…", zh: "正在逐个测试 {0} 个模型…", "\(routes.count)")
        ...
        modelTestTask = Task { [weak self] in
            ...
            for route in routes {
                guard !Task.isCancelled else {
                    if endpointOperationToken == operationToken {
                        statusMessage = L10n.t("Model verification was cancelled", zh: "模型验证已取消")
                    }
                    return
                }
                do {
    ...
    func configureOpenCode(
        ...
        do {
            ...
            if launch {
                ...
                statusMessage = L10n.t("OpenCode configuration generated", zh: "OpenCode 配置已生成")
            }
        } catch {
            statusMessage = error.localizedDescription
        }
    ...
    func checkForUpdates(silent: Bool = false) async {
        ...
        if !silent { statusMessage = L10n.t("Checking GitHub for updates…", zh: "正在检查 GitHub 更新…") }
        ...
        do {
            ...
            if !release.draft, !release.prerelease,
               VersionComparator.isNewer(release.version, than: currentVersion) {
                ...
                statusMessage = L10n.t("New version {0} is available", zh: "发现新版本 {0}", release.version)
            } else {
            ...
            if !silent {
                statusMessage = L10n.t("Update check failed: {0}", zh: "更新检查失败：{0}", message)
            }
        }
    }
    ...
    func resetUpdateInstallerHandoff() {
        ...
        updateInstallerWasOpened = false
        statusMessage = L10n.t("You can restart the update installer", zh: "可以重新启动更新安装器")
    }
    ...
    func startProbe() {
        ...
        do {
            try proxy.start()
            statusMessage = L10n.t("Starting the probe proxy…", zh: "正在启动探测代理…")
        } catch {
            statusMessage = L10n.t("Proxy failed to start: {0}", zh: "代理启动失败：{0}", error.localizedDescription)
            self.proxy = nil
    ...
    func removeAnthropicBridge() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                statusMessage = L10n.t("Bridge uninstall failed: {0}", zh: "Bridge 卸载失败：{0}", error.localizedDescription)
            }
            isBusy = false
        }
    }
    ...
    func copyCursorBaseURL() {
        ...
        statusMessage = L10n.t("Copied the Cursor Override OpenAI Base URL", zh: "已复制 Cursor Override OpenAI Base URL")
    }
    ...
    func copyCursorAPIKey() {
        ...
        guard !key.isEmpty else {
            statusMessage = L10n.t("This endpoint has no API key to copy", zh: "当前 Endpoint 没有可复制的 API Key")
            return
        }
    ...
    func copyCursorModelIDs() {
        ...
        statusMessage = L10n.t("Copied {0} model IDs", zh: "已复制 {0} 个模型 ID", "\(modelIDs.count)")
    }
    ...
    func clearDiagnostics() {
        events.removeAll()
        statusMessage = L10n.t("Diagnostics cleared", zh: "诊断记录已清除")
    }
